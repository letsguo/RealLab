...
class Hound_MPPI_HL_Control:
    def __init__(self, mppi_config: MPPIConfig):
        ...
        # self.mppi_config = Config["MPPIConfig"]
        # self.cost_config = Config["CostConfig"]
        # self.dynamics_config = Config["DynamicsConfig"]
        # self.sampling_config = Config["SamplingConfig"]
        ...
        # make sure mppi_config contains the minimal_cost config
        # TODO: might need to doulbe check the dtype and device
        ...
        self.map_size = self.map_config.map_length_px
        self.map_res = self.map_config.map_res_m_px
        # ## state variables
    ...
    def main_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (
                ...
                print("POSITION: ", self.state[:2])
                pos_error = np.linalg.norm(self.state[:2] - self.goal[:2])
                vel_error = np.linalg.norm(self.state[3:5])
                terminate = (
                    pos_error < self.goal_tolerance and
                    vel_error < self.velocity_tolerance
                ) or (time.time() - self.start_time > self.max_duration)
    ...
    # currently not needed for fixed goal I believe
    def update_goal(
        self,
        goal,
        pos,
...
if __name__ == "__main__":
    ...
    Config = MPPIConfig()
    planner = Hound_MPPI_HL_Control(Config)
    rospy.spin()
